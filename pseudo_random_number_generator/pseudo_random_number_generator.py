...
class PseudoRandomNumber(object):
    ...
    s_array_of_xoshiro256plusplus: np.ndarray
    ...
    def __init__(self, seed: int = None, prng_type: str = 'xoshiro256++'):
        ...
        if isinstance(seed, int):
            assert seed >= 0, "Error: seed can't be negative."
        assert prng_type in self.prng_object_dict.keys(), 'Error: string for prng_type not in the specified list.'

        self.seed = self.__seed_initialization(seed, prng_type)
        self.prng_type = prng_type

    def __seed_initialization(self, seed: int, prng_type: str):
        # The original seed is hash obfuscated for pseudo-random generation.
        from math import ceil
        from hashlib import blake2b

        # TODO Fix mistake: py shadow names should not be used.
        ...
        # TODO Fix mistake: py shadow names should not be used.
        # noinspection PyShadowingNames
        def initializes_seed_for_xoshiro256plusplus(seed: int, prng_type: str):
            # Generate the self variable used by the Xoshiro256PlusPlus algorithm.
            byte_length_of_seed = ceil(
                self.__class__.seed_length_dict[prng_type] / 8)  # Converts bit length to byte length.
            ...
            while True:  # The Xoshiro256PlusPlus algorithm requires that the input seed value is not zero.
                hash_seed_bytes = blake2b(seed.to_bytes(byte_length_of_seed, byteorder='little'),
                                          digest_size=blake2b_digest_size).digest()
            ...
            self.s_array_of_xoshiro256plusplus = np.array(np.frombuffer(hash_seed_bytes, dtype=np.uint64))
            # Xoshiro256PlusPlus to use the 256 bit uint64 seed array.

        each_algorithm_init_dict = {'xoshiro256++': initializes_seed_for_xoshiro256plusplus}

        if seed is None:
            from secrets import randbits
            seed = randbits(self.__class__.seed_length_dict[prng_type])
            # Read unreproducible seeds provided by the operating system.
        else:
            seed = seed_length_mask(seed, prng_type)

        # The specific initialization seed method is called according to prng_type.
        each_algorithm_init_dict[prng_type](seed, prng_type)

        return seed

    @np.errstate(over='ignore')
    def __xoshiro256plusplus(self) -> int:
        # Xoshiro256PlusPlus method realizes full-period length output, [1, 2^ 256-1]
        ...
        for i in range(4):
            result |= int(
                rotl(self.s_array_of_xoshiro256plusplus[0] + self.s_array_of_xoshiro256plusplus[3], np.uint64(23)) +
                self.s_array_of_xoshiro256plusplus[0]) << (64 * i)
    ...
    def source_random_number(self):
        """
            ...
            Note
            ----
            The value type and value range are determined by the algorithm specified by the prng_type parameter
    ...
    def rand_float(self, new_period: int = None) -> gmpy2.mpfr:
        ...
        with gmpy2.local_context(gmpy2.context(), precision=bit_num_of_prng_period + 1):
    ...
    def random_integer_number_with_definite_period(self, new_period: int) -> int:
        """
            ...
            Note
            ----
            The value of (new period / original period) is the conversion efficiency.
            When the difference between the new period and the original period is large,
            generating a pseudo-random number can be very slow!

            Examples
            --------
            >>> random = PseudoRandomNumber(17014118346046923173167303715884105727)
        ...
        assert new_period <= prng_period, \
            'Error: Suppose the new period number cannot be greater than ' \
            'the original period number of the pseudorandom number generator.'

        number_to_subtract = prng_period - new_period
        set_of_numbers_to_exclude = self.generate_set_of_integer_random_numbers(
            prng_period, prng_period & 1, number_to_subtract)
